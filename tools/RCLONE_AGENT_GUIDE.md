...
```
При повторной заливке (перезаписи существующего видео тем же `copyto` в тот же путь) ссылка, как правило, не меняется: `--yandex-upload-wait 2s` только ждёт, чтобы Диск успел принять новое содержимое по существующему пути, а сама публичная ссылка в Яндекс.Диске привязана к пути/ресурсу, а не к конкретной версии содержимого. Если ресурс по этому пути уже был опубликован раньше, `rclone link` не создаёт новую публикацию — он возвращает уже существующий публичный токен, который теперь отдаёт новый файл. Если ссылка нужна для внешнего сообщения (README, чат) — всё равно вызывать `rclone link` заново после `copyto` и не полагаться на ранее сохранённый URL вслепую: он не изменится сам по себе, но убедиться стоит.
