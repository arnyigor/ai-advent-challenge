"""Persistent Day 9 transcript, request log and separate rolling summary."""
...
class SQLiteHistoryStore:
    ...
    def load_summary(self, session_id: str) -> dict[str, Any] | None:
        with self._lock, self._connect() as conn:
            row = conn.execute(
                "SELECT content, summarized_message_count, revision, usage_json, updated_at "
    ...
    def save_summary(
        ...
        *,
        ...
        with self._lock, self._connect() as conn, conn:
            conn.execute(
                "INSERT INTO summaries (session_id, content, summarized_message_count, "
                "revision, usage_json) VALUES (?, ?, ?, ?, ?) "
