"""LLM providers reused by Day 9 for answers and summary generation."""
...
from tools.llm.huggingface import (
    DEFAULT_MODEL as HF_DEFAULT_MODEL,
    HuggingFaceFatalError,
    HuggingFaceModelUnavailableError,
...
def _looks_like_context_limit_error(text: str) -> bool:
    lowered = text.lower()
...
class ClientProvider:
    ...
    def prepare_prompt(self, messages: Sequence[Message], _config: AgentConfig) -> str:
        return _conversation_prompt(messages)
    ...
    def generate(
        ...
        prompt = self.prepare_prompt(messages, config)
        ...
        try:
            ...
            if _looks_like_context_limit_error(str(exc)):
                raise ProviderContextLimitError(f"{self.label}: {exc}") from None
...
class HuggingFaceProvider(ClientProvider):
    """Qwen2.5-Coder-7B via the HF Router (nscale): native 32K context, far
    smaller than DeepSeek's 1M window, so a genuine overflow test (Day 8
    scenario C) fits in one request instead of megabytes of paid traffic."""

    def __init__(self):
        super().__init__(
            provider_id="hf",
            label="HuggingFace",
            model_chain=[HF_DEFAULT_MODEL],
            fallback_exc=(
                HuggingFaceFatalError,
                HuggingFaceRetryableError,
                HuggingFaceModelUnavailableError,
            ),
            has_key=has_hf_token,
        )
...
class OpenAICompatibleProvider:
    ...
    def generate(
        ...
        if response.status_code != 200:
            if _looks_like_context_limit_error(response.text):
                raise ProviderContextLimitError(
                    f"{self.label}: HTTP {response.status_code}: {response.text[:300]}"
                )
