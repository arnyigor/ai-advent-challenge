"""Local Day 9 web chat with observable context compression."""
            ...
            self._json(
                {
                    ...
                    "summary": agent.summary.to_dict(),
                }
            )
            return
        if parsed.path == "/api/comparison":
            comparison_path = DAY_DIR / "results" / "comparison.json"
            if not comparison_path.exists():
                self._json({"error": "Comparison has not been run yet"}, HTTPStatus.NOT_FOUND)
                return
            try:
                comparison = json.loads(comparison_path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError):
                self._json({"error": "Comparison result is invalid"}, HTTPStatus.INTERNAL_SERVER_ERROR)
                return
...
def main():
    parser = argparse.ArgumentParser(description="Day 9 context compression web chat")
    parser.add_argument("--host", default=os.environ.get("DAY09_WEB_HOST", "127.0.0.1"))
    parser.add_argument("--port", type=int, default=int(os.environ.get("DAY09_WEB_PORT", "8009")))
    args = parser.parse_args()
    server = create_server(args.host, args.port)
    host, port = server.server_address
    print(f"Day 9 agent: http://{host}:{port}/")
    try:
