...
const port = Number(process.env.DAY07_WEB_PORT || 8071);
...
const cdpPort = Number(process.env.DAY07_CDP_PORT || 9261);
const pythonPath = process.env.PYTHON_PATH || 'python';
const output = process.argv.includes('--out')
  ? path.resolve(process.argv[process.argv.indexOf('--out') + 1])
  : path.resolve(rootDir, 'ChallengeVideos', 'day07-demo.mp4');
mkdirSync(path.dirname(output), {recursive: true});

// Clean slate: the video's "agent remembers" claim must come from this run's
// own restart, not from a stale .db left over by an earlier take.
...
async function requireAnswer(expectedCount, label) {
  const ok = await waitFor(`document.querySelectorAll('.turn:not(.me) .md').length>=${expectedCount}`);
...
async function main() {
  ...
  await scene('Day 07 · Сохранение контекста — история диалога хранится в SQLite, а не только в памяти процесса', null, 5000);

  await evaluate(sendMessageScript('Запомни: мой любимый цвет — фиолетовый.'));
  await scene('Начинаем диалог: просим агента запомнить факт', null, 2000);
  await requireAnswer(1, 'Ответ на первый факт');
  await scene('Агент подтвердил факт — он уже записан в chat_history.db, не только в оперативной памяти', null, 5000);

  await scene('Останавливаем процесс web_server.py целиком — это перезапуск приложения, не кнопка «Очистить контекст»', null, 3000);
  await new Promise(resolve => { server.once('exit', resolve); server.kill(); });
  await scene('Процесс завершён — вся оперативная память сервера уничтожена вместе с ним', null, 3000);

  server = startServer();
  await waitJson(`http://127.0.0.1:${port}/api/config`);
  await scene('Запускаем web_server.py заново — это свежий процесс, у него нет никакой истории в памяти', null, 2500);

  await cdp('Page.navigate', {url: pageUrl});
  await waitFor("document.getElementById('providerSelect')!==null && providerSelect.options.length>1");
  await scene('Обновляем страницу — фронтенд запрашивает GET /api/history у нового процесса', null, 3000);
  const restoredOk = await waitFor("document.querySelectorAll('.turn').length>=2");
  if (!restoredOk) throw new Error('История не восстановилась в интерфейсе после перезапуска сервера');
  await scene('Прошлые реплики на месте — они пришли из SQLite, а не из памяти (той уже не существует)', null, 5000);

  await evaluate(sendMessageScript('Какой мой любимый цвет? Ответь одним словом.'));
  await scene('Продолжаем диалог вопросом, ответ на который знает только прошлый (уже мёртвый) процесс', null, 2000);
  await requireAnswer(2, 'Ответ после перезапуска');
  await scene('Агент верно назвал цвет — контекст пережил полный перезапуск приложения', null, 8000);
...
main().catch(error => { recording = false; try { chrome.kill(); } catch {} try { server.kill(); } catch {} console.error(error.message); process.exit(1); });
